...
import { DeliveryRatesService } from '@/shared/api/services/delivery-rates.service';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    if (req.method === 'GET') {
      const rates = await DeliveryRatesService.getAll();
      return res.status(200).json({ data: rates });
    }

    if (req.method === 'POST') {
      const created = await DeliveryRatesService.create(req.body);
    ...
    if (req.method === 'PATCH') {
      ...
      if (action === 'toggle-active') {
        const updated = await DeliveryRatesService.toggleActive(uuid, req.body.isActive);
        return res.status(200).json({ data: updated });
      }

      const updated = await DeliveryRatesService.update(uuid, req.body);
      return res.status(200).json({ data: updated });
    }
