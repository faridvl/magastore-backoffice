        ...
        <div className="max-w-4xl mx-auto space-y-8">
                ...
                <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-3">
                    <div className="relative group flex-1">
                        <input
                            ...
                            className="w-full p-5 pl-14 bg-slate-50 dark:bg-slate-800 border-2 border-transparent focus:border-amber-500 rounded-3xl outline-none transition-all font-mono text-base"
                        />
                        <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-amber-500" size={22} />
                    </div>
                    <button
                        type="submit"
                        disabled={isFetching}
            ...
            {data && (
                <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-100 dark:border-slate-800 shadow-sm">
                            ...
                            <div className="space-y-4">
                                ...
                                <div>
                                    <p className="text-[10px] font-black text-slate-400 uppercase">Casillero</p>
                        ...
                        <div className="bg-slate-900 p-8 rounded-[2.5rem] text-white shadow-xl">
                            ...
                            <div className="space-y-4">
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className={`text-xl font-black ${sinFactura ? 'text-slate-500' : 'text-white'}`}>
                                        {sinFactura
                                            ? '—'
                                ...
                                {data.delivery_method && (
                                    <div className="flex justify-between items-center pt-2 border-t border-slate-800">
                                        <span className="text-xs text-slate-500">Método de entrega:</span>
                                        <span className="text-xs text-slate-300">{data.delivery_method}</span>
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    {/* Detalle técnico */}
                    <div className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-100 dark:border-slate-800 flex flex-wrap gap-8 justify-around">
                        <div className="text-center">
                            <p className="text-[10px] font-black text-slate-400 uppercase">Peso Real</p>
                            <p className="font-bold">{data.weight_lb ? `${data.weight_lb} lbs` : '—'}</p>
                        </div>
                        <div className="text-center">
                            <p className="text-[10px] font-black text-slate-400 uppercase">UUID</p>
                            <p className="font-mono text-xs text-slate-500">{data.uuid}</p>
                        </div>
                    </div>
